...
```{r}
training_geral <- bake(receita_prep, new_data = NULL)
test_geral <- bake(receita_prep, new_data = test)
```
...
```{r}
training_proc <- subset(training_geral, select = -c(type))
test_proc <- subset(test_geral, select = -c(type))
```
...
E por último vamos definir a matriz de confusão desse modelo de regressão multinomial:
```{r}
matriz_confusao_glm <- confusionMatrix(fitted_glm$.pred_Very_Good, fitted_glm$observado)
```


### Avaliação dos Modelos Supervisionados

Os resultados finais dos modelos acima estimados com tidymodels estão abaixo:
...
Por ultimo, Matrizes de confusão para comparar o número de acertos:

```{r}


# Matriz do modelo Multinomial
conf_matrix_test <- 
  confusionMatrix(fitted_mt$observado, fitted_mt$.pred_class)

conf_matrix_test

# Matriz do modelo XGboost
conf_matrix_xgb <- 
  confusionMatrix(fitted_bst$observado, fitted_bst$.pred_class)

conf_matrix_xgb


# Matriz do modelo Rede Neural
conf_mat_rn <- 
  table(fitted_net$observado, fitted_net$.pred_class)

conf_mat_rn

# Deu erro, não consegui utilizar o mesmo modelo das anteriores para a RN
#conf_matrix_rn <- 
#  confusionMatrix(fitted_net$observado, fitted_net$.pred_class)
...
# retorna o nome da primeira componente principal
contribuicoes_primeira_componente <- abs(pca$rotation[, 1])

contribuicoes_primeira_componente
...
(descricao <- dados %>% 
    mutate(cluster = factor(kmeans(coffee_cluster, centers = , nstart = 10)$cluster)))
```
